...
class RestaurantOrderManagement:
    def __init__(self, root):
        self.root = root
        self.root.title("Restaurant Management Application")
        self.menu_items = {"FRIES MEAL": 2,"LUNCH MEAL": 2,"BURGER MEAL": 3,"PIZZA MEAL": 4,"CHEESE BURGER": 2.5,"DRINKS": 1}
        self.exchange_rate = 82
        self.setup_background(root)
        frame = ttk.Frame(root)
        frame.place(relx=0.5, rely=0.5,anchor=tk.CENTER)
        ttk. Label(frame, text="Restaurant Order Management",font=( "Artal", 20, "bold")).grid(row=0, columnspan=3, padx=10, pady=10)
        self.menu_labels = {}
        self.menu_quantittes = {}
        for i, (item, price) in enumerate(self.menu_items.ttems(), start=1):
            ...
            currency_dropdown = ttk.Combobox(frame, text_variable=self.currency_var, state="readonly", width=18, values=( 'USD', 'INR' ))
